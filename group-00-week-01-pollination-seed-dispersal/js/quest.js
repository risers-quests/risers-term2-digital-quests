...
(function () {
  ...
  function initMaterialsPool(pageKey) {
    ...
    var storageKey = 'imm-l3-w1-materials::' + pageKey;
  ...
  function initBuildChecklist(pageKey) {
    ...
    if (!items.length) return;
    var storageKey = 'imm-l3-w1-build::' + pageKey;
  ...
  function initFieldAutosave(pageKey) {
    var storageKey = 'imm-l3-w1-fields::' + pageKey;
  ...
  function initProgressBar(pageKey) {
    ...
    function reflectFilled(id) {
      var s = loadJSON('imm-l3-w1-reflect::' + pageKey + '::' + id, null);
      return !!(s && s.text && s.text.trim());
    }

    function recompute() {
      // Day 1 now carries 2 checks per reading section rather than 1, and the
      // exact extra ids can vary slightly per kid, so detect whichever of
      // these actually exist on the page rather than assuming a fixed list.
      var day1Candidates = ['refl-1', 'refl-2', 'refl-3', 'refl-4', 'refl-6', 'refl-7', 'refl-8', 'refl-9', 'refl-10', 'refl-11'];
      ...
      var buildState = loadJSON('imm-l3-w1-build::' + pageKey, {});
  ...
  function initDayLock(pageKey) {
    ...
    function dayDone(day) {
      ...
      if (day.buildIndices.length) {
        var state = loadJSON('imm-l3-w1-build::' + pageKey, {});
  ...
  function initHighlighter(pageKey) {
    ...
    var storageKey = 'imm-l3-w1-hl::' + pageKey;
  ...
  function initReflectionChecks(pageKey, configs) {
    ...
    configs.forEach(function (cfg) {
      ...
      var storageKey = 'imm-l3-w1-reflect::' + pageKey + '::' + cfg.id;
  ...
  function initNotesDrawer(pageKey) {
    ...
    var storageKey = 'imm-l3-w1-notes::' + pageKey;
  ...
  function initMatchGame(containerId, pairs, opts) {
    ...
    if (!container) return;
    opts = opts || {};
    var storageKey = 'imm-l3-w1-game::' + (opts.pageKey || '') + '::' + containerId;
    var saved = loadJSON(storageKey, null);
  ...
  function initCompleteQuest(pageKey) {
    ...
    var doneKey = 'imm-l3-w1-quest-completed::' + pageKey;
  ...
  function collectSyncState(pageKey) {
    var state = {
      build: loadJSON('imm-l3-w1-build::' + pageKey, {}),
      materials: loadJSON('imm-l3-w1-materials::' + pageKey, null),
      fields: loadJSON('imm-l3-w1-fields::' + pageKey, {}),
      hl: loadJSON('imm-l3-w1-hl::' + pageKey, []),
      notes: localStorage.getItem('imm-l3-w1-notes::' + pageKey) || '',
      reflect: {},
      dayTime: loadJSON('imm-l3-w1-time::' + pageKey, {}),
      completed: localStorage.getItem('imm-l3-w1-quest-completed::' + pageKey) === '1'
    };
    document.querySelectorAll('textarea[id^="refl-"]').forEach(function (ta) {
      state.reflect[ta.id] = loadJSON('imm-l3-w1-reflect::' + pageKey + '::' + ta.id, null);
    });
    return state;
  }
  ...
  function applySyncState(pageKey, state) {
    if (!state) return;
    if (state.build) saveJSON('imm-l3-w1-build::' + pageKey, state.build);
    if (state.materials) saveJSON('imm-l3-w1-materials::' + pageKey, state.materials);
    if (state.fields) saveJSON('imm-l3-w1-fields::' + pageKey, state.fields);
    if (state.hl) saveJSON('imm-l3-w1-hl::' + pageKey, state.hl);
    if (typeof state.notes === 'string') { try { localStorage.setItem('imm-l3-w1-notes::' + pageKey, state.notes); } catch (e) {} }
    if (state.reflect) {
      Object.keys(state.reflect).forEach(function (id) {
        if (state.reflect[id]) saveJSON('imm-l3-w1-reflect::' + pageKey + '::' + id, state.reflect[id]);
      });
    }
    if (state.dayTime) saveJSON('imm-l3-w1-time::' + pageKey, state.dayTime);
    if (typeof state.completed === 'boolean') {
      try { localStorage.setItem('imm-l3-w1-quest-completed::' + pageKey, state.completed ? '1' : '0'); } catch (e) {}
    }
  }
  ...
  function initDayTimer(pageKey) {
    ...
    var storageKey = 'imm-l3-w1-time::' + pageKey;
  ...
  function initProgressSync(pageKey, group, week) {
    ...
    var syncedAtKey = 'imm-l3-w1-synced-at::' + pageKey;
    ...
    var pendingKey = 'imm-l3-w1-sync-pending::' + pageKey;
