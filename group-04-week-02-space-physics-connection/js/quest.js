...
(function () {
      ...
      btn.addEventListener('click', function () {
        ...
        if (!state.success) {
          ...
          if (!state.success) {
            ...
            hint.style.display = 'none';
            checkMeaningRemote(getPromptText(textarea), spellGroups, text, function (result) {
              if (result.pass) {
                state.success = true;
                state.missingTopics = [];
  ...
  function initSequenceGame(containerId, steps, opts) {
    ...
    if (!container) return;
    opts = opts || {};
    var storageKey = 'imm-l3-w2-seqgame::' + (opts.pageKey || '') + '::' + containerId;
    var saved = loadJSON(storageKey, null);
    var best = (saved && saved.best) || null;
    ...
    var state = {
      ...
      calcGame: (function () {
        ...
        Object.keys(localStorage).forEach(function (k) {
          if (k.indexOf('imm-l3-w2-calcgame::' + pageKey + '::') === 0) out[k] = loadJSON(k, null);
        });
        return out;
      })()
    };
  ...
  function applySyncState(pageKey, state) {
    ...
    if (state.calcGame) {
      Object.keys(state.calcGame).forEach(function (k) {
        if (state.calcGame[k]) saveJSON(k, state.calcGame[k]);
      });
    }
  }
  ...
  function initCalcQuest(containerId, opts) {
    ...
    var storageKey = 'imm-l3-w2-calcgame::' + pageKey + '::' + containerId;
    var FUEL_START = 100;
    var FUEL_PENALTY = 15;
    ...
    function renderComplete() {
      if (opts.completionReflId) {
        saveJSON('imm-l3-w2-reflect::' + pageKey + '::' + opts.completionReflId, { success: true, text: 'Mission complete', attempts: state.attempts.reduce(function (a, b) { return a + b; }, 0) });
      }
      stageArea.innerHTML = '';
