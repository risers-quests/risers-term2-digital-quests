...
(function () {
  ...
  function initMaterialsPool(pageKey) {
    ...
    var storageKey = 'imm-l3-w4-materials::' + pageKey;
  ...
  function initBuildChecklist(pageKey) {
    ...
    if (!items.length) return;
    var storageKey = 'imm-l3-w4-build::' + pageKey;
  ...
  function initFieldAutosave(pageKey) {
    var storageKey = 'imm-l3-w4-fields::' + pageKey;
  ...
  function initProgressBar(pageKey) {
    ...
    function reflectFilled(id) {
      var s = loadJSON('imm-l3-w4-reflect::' + pageKey + '::' + id, null);
    ...
    function recompute() {
      ...
      var buildState = loadJSON('imm-l3-w4-build::' + pageKey, {});
      ...
      // denominator, same as day1Candidates above. buildTotal itself is
      // read straight from the actual .build-check-item count on the page,
      // not a hardcoded 5/6 guess, so a build with a different real step
      // count (this quest's is 6) still computes correctly.
      var hasSelfReview = !!document.getElementById('refl-5');
      var buildTotal = document.querySelectorAll('.build-check-item').length + (hasSelfReview ? 1 : 0);
  ...
  function initDayLock(pageKey) {
    ...
    function dayDone(day) {
      ...
      if (day.buildIndices.length) {
        var state = loadJSON('imm-l3-w4-build::' + pageKey, {});
  ...
  function initHighlighter(pageKey) {
    ...
    var storageKey = 'imm-l3-w4-hl::' + pageKey;
  ...
  function initReflectionChecks(pageKey, configs) {
    ...
    configs.forEach(function (cfg) {
      ...
      var storageKey = 'imm-l3-w4-reflect::' + pageKey + '::' + cfg.id;
  ...
  function initNotesDrawer(pageKey) {
    ...
    var storageKey = 'imm-l3-w4-notes::' + pageKey;
  ...
  function initMatchGame(containerId, pairs, opts) {
    ...
    if (!container) return;
    opts = opts || {};
    var storageKey = 'imm-l3-w4-game::' + (opts.pageKey || '') + '::' + containerId;
    var saved = loadJSON(storageKey, null);
  ...
  /* ---- Sequence Game: click steps into place in the correct order, one
     ...
     Distinct mechanic from the match game above: this is about ordering a
     chain of cause and effect, not pairing two static facts. ---- */
  ...
  function initCompleteQuest(pageKey) {
    ...
    var doneKey = 'imm-l3-w4-quest-completed::' + pageKey;
  ...
  function collectSyncState(pageKey) {
    var state = {
      build: loadJSON('imm-l3-w4-build::' + pageKey, {}),
      materials: loadJSON('imm-l3-w4-materials::' + pageKey, null),
      fields: loadJSON('imm-l3-w4-fields::' + pageKey, {}),
      hl: loadJSON('imm-l3-w4-hl::' + pageKey, []),
      notes: localStorage.getItem('imm-l3-w4-notes::' + pageKey) || '',
      reflect: {},
      game: {},
      calcgame: {},
      quiz: {},
      dayTime: loadJSON('imm-l3-w4-time::' + pageKey, {}),
      completed: localStorage.getItem('imm-l3-w4-quest-completed::' + pageKey) === '1'
    };
    document.querySelectorAll('textarea[id^="refl-"]').forEach(function (ta) {
      state.reflect[ta.id] = loadJSON('imm-l3-w4-reflect::' + pageKey + '::' + ta.id, null);
    });
  ...
  function applySyncState(pageKey, state) {
    if (!state) return;
    if (state.build) saveJSON('imm-l3-w4-build::' + pageKey, state.build);
    if (state.materials) saveJSON('imm-l3-w4-materials::' + pageKey, state.materials);
    if (state.fields) saveJSON('imm-l3-w4-fields::' + pageKey, state.fields);
    if (state.hl) saveJSON('imm-l3-w4-hl::' + pageKey, state.hl);
    if (typeof state.notes === 'string') { try { localStorage.setItem('imm-l3-w4-notes::' + pageKey, state.notes); } catch (e) {} }
    if (state.reflect) {
    ...
    if (state.dayTime) saveJSON('imm-l3-w4-time::' + pageKey, state.dayTime);
    if (typeof state.completed === 'boolean') {
      try { localStorage.setItem('imm-l3-w4-quest-completed::' + pageKey, state.completed ? '1' : '0'); } catch (e) {}
    }
  }
  ...
  function initDayTimer(pageKey) {
    ...
    var storageKey = 'imm-l3-w4-time::' + pageKey;
  ...
  function initProgressSync(pageKey, group, week) {
    ...
    var syncedAtKey = 'imm-l3-w4-synced-at::' + pageKey;
    ...
    var pendingKey = 'imm-l3-w4-sync-pending::' + pageKey;
    ...
    return pull().then(function () {
      ...
      var backfillKey = 'imm-l3-w4-sync-backfill-game-calc::' + pageKey;
  ...
  function initCalcQuest(containerId, opts) {
    ...
    var storageKey = 'imm-l3-w4-calcgame::' + pageKey + '::' + containerId;
    var FUEL_START = 100;
    var FUEL_PENALTY = 15;
    ...
    function renderComplete() {
      if (opts.completionReflId) {
        saveJSON('imm-l3-w4-reflect::' + pageKey + '::' + opts.completionReflId, { success: true, text: 'Mission complete', attempts: state.attempts.reduce(function (a, b) { return a + b; }, 0) });
      }
      stageArea.innerHTML = '';
