...
(function () {
  ...
  function initProgressBar(pageKey) {
    ...
    function recompute() {
      ...
      // Day 2 · Mission Storm Track (calc-quest) — one checkable item, done
      ...
      // optional-extra pattern as refl-5 above. buildTotal itself is read
      // straight from the actual .build-check-item count on the page, not
      // a hardcoded guess, so a build with a different real step count
      // still computes correctly.
      var hasBuildReflection = !!document.getElementById('refl-b1');
      var buildTotal = document.querySelectorAll('.build-check-item').length + (hasSelfReview ? 1 : 0) + (hasBuildReflection ? 1 : 0);
  ...
  function initQuizGame(containerId, questions, opts) {
    ...
    if (!container) return;
    opts = opts || {};
    var storageKey = 'imm-l3-w4-quiz::' + (opts.pageKey || '') + '::' + containerId;
    var saved = loadJSON(storageKey, {});
