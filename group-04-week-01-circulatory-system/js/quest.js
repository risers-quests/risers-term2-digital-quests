...
(function () {
  ...
  function videoKey(pageKey, vid) { return 'imm-l3-w1-video::' + pageKey + '::' + vid; }
  ...
  function initReflectionChecks(pageKey, configs) {
    ...
    configs.forEach(function (cfg) {
      ...
      function runLanguageCheck() {
        ...
        checkGrammarRemote(state.text, spellGroups, function (result) {
          if (result.ok) {
            state.langOk = true;
            persist();
            render();
  ...
  function initSequenceGame(containerId, steps, opts) {
    ...
    if (!container) return;
    opts = opts || {};
    var storageKey = 'imm-l3-w1-seqgame::' + (opts.pageKey || '') + '::' + containerId;
    var saved = loadJSON(storageKey, null);
    var best = (saved && saved.best) || null;
